...
| Datas fora do período da campanha | `format="mixed", dayfirst=True` corrompeu as ISO | duas passadas por formato (`02-limpeza.md`) |
| Muitas linhas sumiram após converter data | `to_datetime` virou `NaT` num dos formatos | duas passadas por formato |
| Modelo com desempenho perfeito (F1 = 1.0) | vazamento: alguma feature deriva do alvo | remova `alcance`, `interacoes`, a própria taxa |
...
| Gráfico desenhado em cima do anterior | faltou `plt.figure()` | abra figura nova antes de cada gráfico |
| Todos os clusters parecem iguais menos um | faltou escalonar | `StandardScaler()` antes do KMeans |
