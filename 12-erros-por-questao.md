...
## ⚠️ Os 4 erros que atravessam TODAS as questões
...
- ❌ Passar de 2 frases.
- ✅ O que a resposta precisa ter: recorte estreito (um festival, 8 semanas,
...
- ❌ `pd.to_datetime(s, errors="coerce")` sozinho → as `dd/mm/yyyy` viram
  `NaT` e você **perde linhas sem perceber**.
- ❌ `format="mixed", dayfirst=True` → **corrompe as ISO** (`2026-08-05` vira
  8 de maio). Não dá erro nenhum.
- ✅ Duas passadas por formato + `fillna` (ver `02-limpeza.md`).
- ✅ Depois confira: `df["data_publicacao"].isna().sum()` e `.min()/.max()`.
