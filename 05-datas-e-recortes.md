...
⚠️ Se a base for a **bruta** (formatos misturados), esse jeito simples perde
ou inverte datas — use as duas passadas do `02-limpeza.md`. Na base
`*_analise.csv` (já tratada) o formato é único e a linha acima basta.
Confira sempre: `df["data_publicacao"].isna().sum()` e `.min()/.max()`.

## Coisas que se extraem de uma data

```python
df["data"].dt.date          # só a data (vira objeto date)
df["data"].dt.day           # dia do mês
df["data"].dt.month         # mês
df["data"].dt.hour          # hora
df["data"].dt.dayofweek     # 0 = segunda ... 6 = domingo
df["data"].dt.day_name()    # "Monday", "Tuesday"...
df["data"].dt.isocalendar().week   # número da semana
df["data"].dt.to_period("W")       # agrupar por semana
```
...
df[df["taxa"].between(5, 10)]                      # intervalo
```
